...
- **Categories** — one markdown file per subject in `data/subjects/` (e.g. `data/subjects/DIS.md`). Frontmatter defines the category's `name`, `color` (hex background for the pill/badge), `accentColor` (hex for the colored bar on top of each card), and `cardGradientFrom` / `cardGradientTo` (hex gradient for the card background). Text on colored pills is chosen automatically for contrast. Colors are chosen to be clearly distinguishable from each other.
