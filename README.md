...
- **Categories** — one markdown file per subject in `data/subjects/` (e.g. `data/subjects/DIS.md`). Frontmatter defines the category's `name`, `color` (pill/badge classes), `accentColor` (the colored bar on top of each card), and `cardGradient` (card background). Colors are chosen to be clearly distinguishable from each other.
